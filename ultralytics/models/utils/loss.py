...
class BalancedL1Loss(nn.Module):
    ...
    This is the official implementation from Libra R-CNN:
    "Libra R-CNN: Towards Balanced Learning for Object Detection"

    Args:
        alpha (float): α in Balanced L1. Default: 0.5.
        gamma (float): γ in Balanced L1. Default: 1.5.
        beta (float): transition point (usually 1.0)
    ...
    def balanced_l1_loss(self, pred, target):
        ...
        # Balanced L1 official piecewise form:
        # if diff < beta:
        #     loss = alpha/b * (b*diff + 1) * ln(b*diff/beta + 1) - alpha*diff
        # else:
        #     loss = gamma*diff + gamma/beta - alpha

        loss = torch.where(
            diff < beta,
            alpha / b * (b * diff + 1) * torch.log(b * diff / beta + 1) - alpha * diff,
            gamma * diff + gamma / beta - alpha)

        return loss

    def forward(self, pred, target, weight=None):
        loss = self.balanced_l1_loss(pred, target)
...
class DETRLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.balanced_l1 = BalancedL1Loss(alpha=0.5, gamma=1.5, beta=0.5, reduction='sum', loss_weight=1.0)
        ...
        if self.use_wiseiou:
            self.wiou_loss = WiseIouLoss(ltype='WPDIoU', monotonous=False, inner_iou=False, focaler_iou=False)
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=''):
        ...
        # 使用BalancedL1Loss：
        balanced_l1_loss = self.balanced_l1(pred_bboxes, gt_bboxes)
        loss[name_bbox] = self.loss_gain['bbox'] * balanced_l1_loss / len(gt_bboxes)
