...
from typing import Optional, Callable, Optional, Dict, Union, List,Tuple
...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'Converse2DC3', 'Converse2D', 'C2f_GCConv', 'GCConvC3', 'GCConv', 'C2f_CFBlock','LGABlock','GLGBlock'
           ]
...
# ----------------------------- Window Attention (lightweight) -----------------------------
class WindowAttention(nn.Module):
    def __init__(self, dim: int, window_size: int = 7, num_heads: int = 4, qkv_bias: bool = True):
        super().__init__()
        ...
        self.relative_bias = nn.Parameter(torch.zeros((2 * window_size - 1) * (2 * window_size - 1), num_heads))
        nn.init.trunc_normal_(self.relative_bias, std=0.02)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, C, H, W = x.shape
        ws = self.window_size
        windows, Hp, Wp = window_partition(x, ws)
        Bwin = windows.shape[0]
        qkv = self.qkv(windows).view(Bwin, 3, C, ws * ws)
        q, k, v = qkv[:, 0], qkv[:, 1], qkv[:, 2]
        q = q.view(Bwin, self.num_heads, self.head_dim, ws * ws).permute(0, 1, 3, 2)
        k = k.view(Bwin, self.num_heads, self.head_dim, ws * ws).permute(0, 1, 3, 2)
        v = v.view(Bwin, self.num_heads, self.head_dim, ws * ws).permute(0, 1, 3, 2)
        ...
        relative_coords = relative_coords.permute(1, 2, 0).contiguous().view(ws * ws, ws * ws, 2)
        rel_h = relative_coords[..., 0] + ws - 1
        rel_w = relative_coords[..., 1] + ws - 1
        idx = rel_h * (2 * ws - 1) + rel_w
        bias = self.relative_bias[idx.view(-1)].view(ws * ws, ws * ws, -1).permute(2, 0, 1)
        attn = attn + bias.unsqueeze(0)
        attn = attn.softmax(dim=-1)
        out = attn @ v
        out = out.permute(0, 1, 3, 2).contiguous().view(Bwin, C, ws * ws)
        out = out.view(Bwin, C, ws, ws)
        out = self.proj(out)
        x_out = window_reverse(out, ws, Hp, Wp, B)
        x_out = x_out[:, :, :H, :W]
        return x_out
...
# ----------------------------- GLGBlock: BiFormer-compatible wrapper -----------------------------
class GLGBlock(nn.Module):
    """Drop-in replacement block that matches BiFormerBlock interface.
    Signature: GLGBlock(ch_in, ch_out, stride, shortcut, act='relu', variant='d')
    Provides `expansion = 1` attribute used by model parser.

    Internally: follows BiFormer style: branch2a -> branch2b -> attention; plus residual/shortcut handling.
    The attention is implemented as a lightweight local-window-global fusion applied after branch2b.
    """
    expansion = 1  # required by parse_model that expects block_type.expansion

    def __init__(self, ch_in, ch_out, stride=1, shortcut=True, act='relu', variant='d',
                 window_size=7, local_kernel=3, dw_expand=1.5, global_reduction=4, num_heads=4):
        super().__init__()
        self.shortcut = shortcut
        # maintain same shortcut behavior as BiFormer
        if not shortcut:
            if variant == 'd' and stride == 2:
                self.short = nn.Sequential(
                    nn.AvgPool2d(2, 2, 0, ceil_mode=True),
                    ConvNormLayer(ch_in, ch_out, 1, 1)
                )
            else:
                self.short = ConvNormLayer(ch_in, ch_out, 1, stride)

        # branch convs similar to BiFormer
        self.branch2a = ConvNormLayer(ch_in, ch_out, 3, stride, act=act)
        self.branch2b = ConvNormLayer(ch_out, ch_out, 3, 1, act=None)
        self.act = nn.Identity() if act is None else get_activation(act)

        # attention / GLG fusion built to accept branch2b output channels
        mid = max(1, int(ch_out * dw_expand))
        # local branch (pointwise -> depthwise -> pointwise)
        self.local_pw1 = nn.Conv2d(ch_out, mid, kernel_size=1, bias=False)
        self.local_bn1 = nn.BatchNorm2d(mid)
        self.local_dw = nn.Conv2d(mid, mid, kernel_size=local_kernel, padding=local_kernel // 2, groups=mid, bias=False)
        self.local_bn2 = nn.BatchNorm2d(mid)
        self.local_pw2 = nn.Conv2d(mid, ch_out, kernel_size=1, bias=False)
        self.local_bn3 = nn.BatchNorm2d(ch_out)

        # window attention on local features
        self.win_attn = WindowAttention(dim=ch_out, window_size=window_size, num_heads=num_heads)

        # global branch
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        self.global_fc = nn.Sequential(
            nn.Linear(ch_out, ch_out // global_reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(ch_out // global_reduction, ch_out, bias=False),
            nn.Sigmoid()
        )

        # fusion conv
        self.fuse = nn.Sequential(
            nn.Conv2d(ch_out * 3, ch_out, kernel_size=1, bias=False),
            nn.BatchNorm2d(ch_out),
        )

        # channel attention
        self.eca = ECA(ch_out, k_size=3)

    def forward(self, x):
        out = self.branch2a(x)
        out = self.branch2b(out)  # (B, ch_out, H', W')

        # local branch
        xl = self.local_pw1(out)
        xl = self.local_bn1(xl)
        xl = F.relu(xl)
        xl = self.local_dw(xl)
        xl = self.local_bn2(xl)
        xl = F.relu(xl)
        xl = self.local_pw2(xl)
        xl = self.local_bn3(xl)
        xl = F.relu(xl)

        # window attention applied on xl (produces same shape)
        xw = self.win_attn(xl)

        # global branch
        g = self.global_pool(xl).flatten(1)
        gg = self.global_fc(g).view(-1, xl.shape[1], 1, 1)
        xg = xl * gg

        fused = torch.cat([xl, xw, xg], dim=1)
...
# ----------------------------- Blocks wrapper (compat) -----------------------------
class Blocks(nn.Module):
    def __init__(self, out_ch: int, block_type=GLGBlock, repeats: int = 2, stage_num: int = 1, act: str = 'relu'):
        super().__init__()
        layers = []
        for i in range(repeats):
            # follow BiFormer style: first block may need stride or shortcut, but yaml generator handles that
            layers.append(block_type(out_ch, out_ch, stride=1, shortcut=True, act=act))
...
if __name__ == '__main__':
    ...
    x_p4 = torch.randn(2, 256, 64, 64).to(device)
    block_p4 = GLGBlock(256, 256, stride=1, shortcut=True).to(device)
    y_p4 = block_p4(x_p4)
    print('P4 out:', y_p4.shape)
    x_p5 = torch.randn(2, 512, 32, 32).to(device)
    block_p5 = GLGBlock(512, 512, stride=1, shortcut=True).to(device)
    y_p5 = block_p5(x_p5)
    print('P5 out:', y_p5.shape)
...
class GhostConv(nn.Module):
    """
    Ghost Convolution module for efficient multi-scale feature extraction.
    Adapted from official GhostNet GhostModule to match custom ConvNormLayer.
    """

    def __init__(self, ch_in, ch_out, kernel_size=1, stride=1, ratio=2, dw_kernel=5, act='relu'):
        super().__init__()

        # 计算主通道和ghost通道（与官方一致）
        init_channels = math.ceil(ch_out / ratio)
        ghost_channels = init_channels * (ratio - 1)

        # 主卷积路径：使用ConvNormLayer适配您的环境
        self.primary_conv = ConvNormLayer(
            ch_in,
            init_channels,
            kernel_size,
            stride,
        ...
        # Ghost操作路径：深度卷积，groups=init_channels实现depthwise
        self.ghost_conv = ConvNormLayer(
            init_channels,
            ghost_channels,
            dw_kernel,
            1,
            groups=init_channels,  # depthwise卷积
            act=act
        )

        # 可选：添加shortcut以增强残差学习（类似于BiFormerBlock）
    ...
    def forward(self, x):
        ...
        # 切片确保精确通道数（官方机制）
        out = out[:, :self.primary_conv.conv.out_channels + self.ghost_conv.conv.out_channels, :, :]
...
# 文件示例： ultralytics/nn/extra_modules/lgab.py
import torch
import torch.nn as nn
import torch.nn.functional as F
...
class LGABlock(nn.Module):
    ...
    def __init__(self, in_channels=None, out_channels=None, kernel_size=3, *args, **kwargs):
        """
        *args, **kwargs 是为了兼容 Blocks 工厂传入的 stride/act/groups 等参数
        """
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.dw_conv is None:
            ch = self.in_channels if self.in_channels is not None else C
            self.dw_conv = nn.Conv2d(ch, ch, kernel_size=self.kernel_size,
                                     padding=self.padding, groups=ch, bias=False).to(x.device)

        if self.pw_conv is None:
            out_ch = self.out_channels if self.out_channels is not None else C
            self.pw_conv = nn.Conv2d(C, out_ch, 1, bias=False).to(x.device)

        # 深度卷积
        if self.dw_conv.weight.shape[0] != C:
            weight = self.dw_conv.weight[:C, :, :, :]
            x = F.conv2d(x, weight, bias=None, stride=1, padding=self.padding, groups=C)
        else:
            x = self.dw_conv(x)

        # pointwise conv
        if self.pw_conv.weight.shape[0] != x.shape[1]:
            self.pw_conv = nn.Conv2d(x.shape[1], self.out_channels, 1, bias=False).to(x.device)
        x = self.pw_conv(x)
